...
use Lkn\HookNotification\Core\Notification\Domain\CustomNotification;
...
final class NotificationFactory extends Singleton
{
    ...
    /**
     * Finds the built-in recipe that supplies parameters/finders for a hook.
     * Returns null if no recipe covers the hook (custom notifications on such
     * hooks cannot currently be instantiated).
     */
    private function findRecipeForHook(Hooks $hook): ?array
    {
     ...
     */
    private function makeCustomNotifications(
        ...
        $rows = $this->customNotificationRepository->all();
        $instances = [];
        ...
        foreach ($rows as $row) {
            ...
            if ($filterByCode !== null && $filterByCode !== $row['code']) {
                continue;
            }

            $hook = Hooks::tryFrom($row['hook']);

            if ($hook === null) {
                continue;
            }

            $recipe = $this->findRecipeForHook($hook);

            if ($recipe === null) {
                continue;
            }

            $rawNotifTemplates = $templatesByNotification[$row['code']] ?? [];

            if ($makeNotificationsWithActiveTemplatesOnly && empty($rawNotifTemplates)) {
                continue;
            }

            $notifInstance = new CustomNotification(
                $row['code'],
                $recipe['category'],
                $hook,
                $recipe['params'],
            ...
            if (is_callable($filterForNotification) && !$filterForNotification($notifInstance)) {
                continue;
            }
    ...
    public function makeByCode(string $notificationCode): ?AbstractNotification
    {
        ...
        $customNotifications = $this->makeCustomNotifications(
            false,
            [$notificationCode => $rawNotifTemplates],
            $notificationCode
        );
    ...
    public function makeAllForHook(Hooks $hook, bool $onlyEnabled = false): array
    {
        ...
        $customNotifications = $this->makeCustomNotifications(
            ...
            $activeTemplates,
            filterForNotification:
                fn (AbstractNotification $notification) => $notification->hook === $hook,
        );

        return [...$manualFileBasedNotifications, ...$customNotifications];
    }
    ...
    public function makeEnabledNotifs(): array
    {
        ...
        $customCodes = array_column($this->customNotificationRepository->allEnabled(), 'code');
        $activeTemplatesForCustom = array_intersect_key($activeTemplates, array_flip($customCodes));

        $activeTemplatesForFileBased = array_diff_key(
            $activeTemplates,
            $activeTemplatesForBuiltIn,
            $activeTemplatesForCustom,
        );
        ...
        return [
            ...
            ...$this->makeCustomNotifications(true, $activeTemplatesForCustom),
        ];
    }
    ...
    private function makeFileBasedNotification(
        ...
        foreach ($this->fileBasedNotificationClassesCache as $class) {
            try {
                ...
                lkn_hn_debug_trace('ERROR instantiating notification class', [
                    'class' => $class,
                    'error' => $th->getMessage(),
    ...
    private function buildNotificationTemplates(array $rawNotifTemplates): array
    {
        return array_map(
            function (array $notifTemplate) {
                // Defaults back to WP_EVO because multi-lang for
                // templates was initially implemented for it.
                // Handle null safely (PHP 8.1 Platforms::tryFrom(null) throws TypeError).
                $platformValue = $notifTemplate['platform'] ?? null;
                $platform = $platformValue !== null
                    ? (Platforms::tryFrom($platformValue) ?? Platforms::WP_EVO)
                    : Platforms::WP_EVO;

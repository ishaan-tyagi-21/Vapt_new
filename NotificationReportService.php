...
final class NotificationReportService
{
    ...
    public function getStatistics(): array
    {
        ...
        $weekAgo = (new DateTime())->modify('-7 days')->format('Y-m-d H:i:s');
        $monthAgo = (new DateTime())->modify('-30 days')->format('Y-m-d H:i:s');

        $sentToday = $this->notificationReportRepository->getSentCountForPeriod($today);
        $sentWeek = $this->notificationReportRepository->getSentCountForPeriod($weekAgo);
        $sentMonth = $this->notificationReportRepository->getSentCountForPeriod($monthAgo);

        $totalMonth = $this->notificationReportRepository->getTotalCountForPeriod($monthAgo);
        $failedMonth = $this->notificationReportRepository->getFailedCountForPeriod($monthAgo);
        $successRate = $totalMonth > 0 ? round(($sentMonth / $totalMonth) * 100, 1) : null;

        $totalAll = $this->notificationReportRepository->getTotalCount();
        ...
        return [
            ...
            'analytics' => [
                ...
                'total_all' => $totalAll,
                'recent_failures' => $this->notificationReportRepository->getRecentFailures(10),
            ],
        ];
    }
}

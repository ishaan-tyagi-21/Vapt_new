...
final class NotificationSender extends Singleton {
    ...
    public function dispatchNotification(
        ...
        try {
            ...
            lkn_hn_debug_trace('ERROR: dispatch failed for ' . $notification->code, [
                'error' => $th->getMessage(),
    ...
    public function send(
        ...
        if ($platformResolverResult instanceof Result) {
            lkn_hn_debug_trace('ERROR: resolver failed for ' . $notification->code, [
                'resolver_code' => $platformResolverResult->code,
                'resolver_msg' => $platformResolverResult->msg,
                'client_locale' => $notification->client->locale,
                'templates_langs' => array_map(fn($t) => $t->lang, $notification->templates),
            ]);
        ...
        if ($platformResponse->status !== NotificationReportStatus::SENT) {
            lkn_hn_debug_trace('ERROR: platform send failed for ' . $notification->code, [
                'status' => $platformResponse->status->value,
                'msg' => $platformResponse->msg,

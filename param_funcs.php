...
/**
 * Returns the ISO currency code (e.g. "INR", "USD") configured as the
 * client's default currency in WHMCS.
 */
function getInvoiceCurrencyCode(int $invoiceId): string
{
    $currency = getInvoiceCurrency($invoiceId);
...
function getClientLastNameByClientId(int $id): string
{
    $name = trim(Capsule::table('tblclients')
        ->where('id', $id)
        ->value('lastname'));
...
function getInvoicePdfUrlByInvocieId(
    ...
    $pdfUrl = moduleUrl() . '/src/Core/temp/invoices_pdf/serve.php?f=' . urlencode($randomFileName);

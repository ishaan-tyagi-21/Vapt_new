...
final class NotificationPlatformResolver
{
    ...
    public function resolve(AbstractNotification $notification)
    {
        ...
        if (!$templateForClientLang) {
            // Bulk messages
            if (!empty($notification->templates) && is_null($notification->templates[0]->lang)) {
                $templateForClientLang = $notification->templates[0];

                if (!$templateForClientLang->platform) {
                    return new Result(
                        'template-has-no-platform',
                        msg: 'Template has no platform configured. Please re-save the notification template.'
                    );
                }
